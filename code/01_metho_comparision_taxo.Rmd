...
meta_g_b_decond_table_rare %>%
  ...
  column_to_rownames("OTU")-> otu_tab
...
data.frame(sample_ID = colnames(otu_table)) %>%
  mutate(brin = str_extract(sample_ID, "R\\d"),
         method = str_extract(sample_ID, "[A-Z+]$")) %>%
...
obj1$data$tax_data %>%
  ...
  ggplot()+
  geom_point(aes(taxon_id, diff_read, col = method))+
  facet_wrap(~diff_is)
