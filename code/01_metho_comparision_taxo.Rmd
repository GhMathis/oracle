...
differential.analysis.Zif <- function(obj = obj3, pair,
                                        ...
                                        nrSamples=1000,
                                        nrThin = 3){
  ## Setup dataa out of metacoder object
  # Extract taxon rank associated to each Metacoder taxon_id
  rank_df = data.frame(taxon_rank = sapply(names(obj$taxa), function(x) obj$taxa[[x]]$rank$name)) %>%
  rownames_to_column("taxon_id")
...
modelName2 <-"model{
  for (i in 1:D[1]) {    # For each OTU
  		for (j in 1:group[1]){ # I both group we test
  			Y[i,j] ~ dpois(exp(alpha[i] - 1000* pi[i,j]))
  				pi[i,j] ~ dbern(p[i]) # zero inflation
  		}
  	  	  
      for (j in (group[1]+1):(group[1]+group[2])){
  			Y[i,j] ~ dpois(exp(alpha[i] + beta[i] - 1000* pi[i,j])) 
  				pi[i,j] ~ dbern(p[i]) # zero inflation
      }
...
postSamples_df2%>%
  ...
  geom_line(aes(Iteration, value , col = as.factor(Chain)), alpha = 0.3)+
  scale_color_manual(values = wesanderson::wes_palette("FantasticFox1", n = 5))
true_val = data.frame(true_val = c(alpha,beta,p), group = c(rep("alpha",10),rep("beta",10),rep("p",10)),taxa_id = as.character(rep(1:10,3) ))
