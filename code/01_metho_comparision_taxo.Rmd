...
Zero inflantion might bias the variance of taxa abudance in each method thus scaling factors are maybe under estimated 
...
x = sort(x, TRUE)
sigtabgen_test$Genus = factor(as.character(sigtabgen_test$genus), levels = names(x))
...
## Bayesian model with zero inflation

```{r}

differential.analysis.Zif <- function(obj = obj3, pair,
                                        nrAdapt=1000,
                                        nrBurnIn=1000,
                                        nrSamples=1000,
                                        nrThin = 3){
  ...
  group = c(rep(0,groups[1]),rep(1,groups[2])) 
  ...
  for(t_rank in c("k", "p", "c", "o", "f", "g")){
    ...
    for (i in 1:dim(Y)[1]){
      ...
      if(sum(Y[i,1:groups[1]])==0) {
        Y[i,sample(1:groups[1],size=1)] <- 1
        pseudoCounts = pseudoCounts + 1
      }
      if(sum(Y[i,(1+groups[1]):(groups[2]+groups[1])])==0) {
  ...
  model2 <- jags.model("code/Zicompar.jags",data = data_list,inits = init_list, n.chains = 1,n.adapt = nrAdapt)
  update(model2,n.iter=nrBurnIn)
  post_sample2 <- coda.samples(model2,c("alpha1", "alpha2", "alpha3"),n.iter = nrSamples,thin=nrThin)
  summary(post_sample2) -> quantile_summary
    
  quantile_summary$quantiles %>%
  data.frame() %>%
  rownames_to_column("Parameter") %>%
  filter(startsWith(Parameter, "alpha3"))%>%
  mutate(signif = case_when(
    sign(X2.5.) != sign(X97.5.) ~ "NS",
    sign(X2.5.) == sign(X97.5.) ~ "sign"
  )) %>%
  select(X2.5., signif, X50.) -> resultdf
...
#load("outputs/result_bayes_obj3.Rdata")
table(pair_wise_taxa_df$signif )
table(obj3$data$diff_analysis_table$PValue>0.05)
obj3$data$diff_analysis_table$taxon_id[1:1434]
obj3$data$diff_analysis_table_zif <- pair_wise_taxa_df
obj3$data$diff_analysis_table_zif$taxon_id <- obj3$data$diff_analysis_table$taxon_id[1:1434]
obj3$data$diff_analysis_table_zif%>%
  mutate(X50. = case_when(signif == "NS" ~ 0,
                    .default  = X50. )) -> obj3$data$diff_analysis_table_zif

obj3%>%
  metacoder::filter_taxa(taxon_ranks == "o", supertaxa = TRUE)%>%
  metacoder::heat_tree(node_label = taxon_names,
          node_size = n_obs, # number of OTUs
          node_color = X50. , # difference between groups
          node_color_interval = c(-max(abs(X50.)), max(abs(X50.))), # symmetric interval
          node_color_range = c("cyan", "gray", "magenta"), # diverging colors
          node_size_axis_label = "OTU count",
          node_color_axis_label = "logFC",
          layout = "davidson-harel", initial_layout = "reingold-tilford",
...
postSamples_df <-  post_sample2 %>% ggs()
postSamples_df%>%
  mutate(group = str_extract(Parameter, "alpha\\d"),
         taxa_id = str_extract(Parameter,"(alpha\\d)\\[(\\d+)\\]", group = 2))%>%
  select(-Parameter)%>%
  pivot_wider(values_from = value, names_from = group)%>%
  #mutate(alpha_diff = alpha1 - alpha2)%>%
  pivot_longer(starts_with("alpha"), names_to = "Parameter", values_to ="value" )-> postSamples_df
postSamples_df%>%
  filter(taxa_id == 7)%>%
  ggplot() +
...
postSamples_df%>%
  filter(taxa_id %in% 1:10)%>%
  ggplot() +
  facet_wrap(~taxa_id)+
  geom_density(aes(value, fill = Parameter), alpha = 0.3)+
  scale_fill_manual(values = wesanderson::wes_palette("FantasticFox1", n = 5))+
  main_theme
```

### Fist estimate mean abudance per OTU supposing a poisson distribution

```{r}
set.seed(1000)
n_sample = 20
n_taxa = 10 
alpha = rnorm(n_taxa,log(20)) # True abundance mean

p = rep(0.8, n_taxa) # probability of presence
pi_ = matrix(rbinom(n_taxa*n_sample,1,p ),nrow = n_taxa, ncol = n_sample)
rowSums(pi_)/100 #should be around 0.8

beta = c(rep(log(0.25), 4), rep(log(1),2), rep(log(4), 4)) # scaling detection factor of group 2 for each taxa
group = c(rep(0,n_sample/2),rep(1,n_sample/2))

lambda = matrix(NA,nrow = n_taxa, ncol = n_sample)
Y = matrix(NA,nrow = n_taxa, ncol = n_sample)
for (t in 1:n_taxa){
  for(s in 1:n_sample){
  
  lambda[t,s] =  exp(alpha[t] + group[s]*beta[t])*pi_[t,s]
  Y[t,s] = rpois(1, lambda[t,s])
  }
}
hist(Y, breaks = 40)
```

# basic model of comparison
```{r}
nrAdapt=1000
nrBurnIn=1000
nrSamples=1000
...
modelName1 <- "model{
  for (i in 1:D[1]) {    # For each OTU
  		for (j in 1:group[1]){ # I both group we test
  			Y[i,j] ~ dpois(exp(alpha[i] ))
  		}
  	  	  
      for (j in (group[1]+1):(group[1]+group[2])){
  			Y[i,j] ~ dpois(exp(alpha[i] + beta[i])) 
      }
      
  		alpha[i] ~ dnorm(0,0.01) # OTU specific true abundance
  		beta[i] ~ dnorm(0,0.01) # OTU diff abundance in grp 2 compaire to grp 1

  }}"

# basic model that estimate the 
data_list = list(
  Y = Y,
  D = dim(Y),
  group = c(10,10)
...
model1 <- jags.model(textConnection(modelName1),data = data_list,inits = init_list, n.chains = 1,n.adapt = nrAdapt)
update(model1,n.iter=nrBurnIn)
post_sample1 <- coda.samples(model1,c("alpha", "beta"),n.iter = nrSamples,thin=nrThin)

summary(post_sample1) -> quantile_summary1
postSamples_df1 <-  post_sample1 %>% ggs()
true_val = data.frame(true_val = c(alpha,beta), group = c(rep("alpha",10),rep("beta",10)),taxa_id = as.character(rep(1:10,2) ))
postSamples_df1%>%
  mutate(group = str_extract(Parameter,"(.+)\\[(\\d+)\\]", group = 1),
         taxa_id = str_extract(Parameter,"(.+)\\[(\\d+)\\]", group = 2))-> postSamples_df1
...
postSamples_df1%>%
  left_join(true_val, by = join_by(group,taxa_id))%>%
  filter(group == "beta")%>%
  ggplot() +
  facet_wrap(~taxa_id)+
  geom_density(aes(value), alpha = 0.3)+
  geom_vline(aes(xintercept = true_val), cex = 2)+
  main_theme

rowMeans(Y)%>%log
quantile_summary1$statistics[11:20,1] %>%exp
```
True abundance mean and group abudance differences aren't well estimated with this model. This is because the model doesn't account for inflated zero in data.
To account for this I will add a probability of observing each taxa in each samples with Bernoulli distribution
...
modelName2 <-"model{
  for (i in 1:D[1]) {    # For each OTU
  		for (j in 1:group[1]){ # I both group we test
  			Y[i,j] ~ dpois(exp(alpha[i] * pi[i,j]))
  			pi[i,j] ~ dbern(p[i]) # zero inflation
  		}
  	  	  
      for (j in (group[1]+1):(group[1]+group[2])){
  			Y[i,j] ~ dpois(exp((alpha[i] + beta[i])* pi[i,j])) 
  				pi[i,j] ~ dbern(p[i]) # zero inflation
      }
...
model2 <- jags.model(textConnection(modelName2),data = data_list,inits = init_list, n.chains = 1,n.adapt = nrAdapt)
update(model2,n.iter=nrBurnIn)
post_sample2 <- coda.samples(model2,c("alpha", "beta"),n.iter = nrSamples,thin=nrThin)
coda.samples(model2,c("p"),n.iter = nrSamples,thin=nrThin)%>%summary

summary(post_sample2) -> quantile_summary2
postSamples_df2 <-  post_sample2 %>% ggs()
postSamples_df2%>%
  mutate(group = str_extract(Parameter,"(.+)\\[(\\d+)\\]", group = 1),
         taxa_id = str_extract(Parameter,"(.+)\\[(\\d+)\\]", group = 2))-> postSamples_df2
...
postSamples_df2%>%
  ...
  geom_line(aes(Iteration, value , col = as.factor(Chain)), alpha = 0.3)+
  scale_color_manual(values = wesanderson::wes_palette("FantasticFox1", n = 5))
true_val = data.frame(true_val = c(alpha,beta,p), group = c(rep("alpha",n_taxa),rep("beta",n_taxa),rep("p",n_taxa)),taxa_id = as.character(rep(1:n_taxa,3) ))
...
postSamples_df2%>%
  ...
  geom_vline(aes(xintercept = true_val, col = group), cex = 2)+
  scale_fill_manual(values = wesanderson::wes_palette("FantasticFox1", n = 3))+
  scale_color_manual(values = wesanderson::wes_palette("FantasticFox1", n = 3))+
  main_theme
quantile_summary2$quantiles[,3]

```

#add complexity

Same type of data but I add :
 - a scaling factor for one method
 - and an over-dispersion parameter per taxa and per sample (more ramdome effect)
...
n_sample = 20
n_taxa = 10 
alpha = rnorm(n_taxa,log(20)) # True abundance mean per taxa
...
# Over dispersion
sigma = round(rgamma(n_taxa, 2,2),2) # taxa specific over-dispersion parameter
u = matrix(rnorm(n_taxa*n_sample, 0, sigma), nrow = n_taxa, ncol = n_sample) # taxa- and sample-specific random effect

beta = c(rep(log(0.25), 4), rep(log(1),2), rep(log(4), 4)) # scaling detection factor between group for each taxa
group = c(rep(0,n_sample/2),rep(1,n_sample/2))
...
for (t in 1:n_taxa){
  for(s in 1:n_sample){
  # method 2 will have 10 time more reads in each samples : group[s]*log(10)
  # 
  lambda[t,s] =  exp(alpha[t] + u[t,s] + group[s]*beta[t] + group[s]*log(10))*pi_[t,s]
  Y[t,s] = rpois(1, lambda[t,s])
  }
}
hist(Y, breaks = 40)
...
With those data the modl 2 should perform badly. because of  the sacling factor of group 2 the value of FC should be mainly positive because 
ab. grp2 > ab. grp1
...
Let's account for sacling factor. I'm add a standartization with respect to row sums to account for sequencing depth of each sample

```{r}
modelName3 <-"model{
  for (i in 1:D[1]) {    # For each OTU
  		for (j in 1:group[1]){ # I both group we test
  			Y[i,j] ~ dpois(exp((alpha[i] + log(N[j])) * pi[i,j]))
  			pi[i,j] ~ dbern(p[i]) # zero inflation
  		}
  	  	  
      for (j in (group[1]+1):(group[1]+group[2])){
  			Y[i,j] ~ dpois(exp((alpha[i] + beta[i] + log(N[j]))* pi[i,j])) 
  				pi[i,j] ~ dbern(p[i]) # zero inflation
      }
...
model3 <- jags.model(textConnection(modelName3),data = data_list,inits = init_list, n.chains = 1,n.adapt = nrAdapt)
update(model3,n.iter=nrBurnIn)
post_sample3 <- coda.samples(model3,c("alpha", "beta"),n.iter = nrSamples,thin=nrThin)
coda.samples(model3,c("p"),n.iter = nrSamples,thin=nrThin)%>%summary

summary(post_sample3) -> quantile_summary3
postSamples_df3 <-  post_sample3 %>% ggs()
postSamples_df3%>%
  mutate(group = str_extract(Parameter,"(.+)\\[(\\d+)\\]", group = 1),
         taxa_id = str_extract(Parameter,"(.+)\\[(\\d+)\\]", group = 2))-> postSamples_df3
...
postSamples_df3%>%
  ...
  scale_color_manual(values = wesanderson::wes_palette("FantasticFox1", n = 3))+
  main_theme
quantile_summary3$quantiles[,3]
beta
```
Now the estimation are still bad (because i don't account for over dispersion per taxa and per sample), but  beta are better etimated that with model 2. They are the same sign that true values, which meas that the standardization worked.

Lets take in consideration over dispersion
...
modelName4 <-"model{
  ...
  for (i in 1:D[1]) {    # For each OTU
  		for (j in 1:group[1]){ # I both group we test
  			Y[i,j] ~ dpois(exp((phi[i,j] + log(N[j])) * pi[i,j]))
  			pi[i,j] ~ dbern(p[i]) # zero inflation
  			phi[i,j] ~ dnorm(alpha[i], binTau[i])
  		}
...
init_list <- list(
  ...
  kappa = 1,
  eta = 1,
  binVar = runif(dim(Y)[1],0.05,0.1),
  phi = alpha + (log(Y+0.1)-alpha)
)

#MCMC run settings

model4 <- jags.model(textConnection(modelName4),data = data_list,inits = init_list, n.chains = 1,n.adapt = nrAdapt)
update(model4,n.iter=nrBurnIn)
post_sample4 <- coda.samples(model4,c("alpha", "beta"),n.iter = nrSamples,thin=nrThin)
coda.samples(model4,c("p"),n.iter = nrSamples,thin=nrThin)%>%summary

summary(post_sample4) -> quantile_summary4
postSamples_df4 <-  post_sample4 %>% ggs()
postSamples_df4%>%
  mutate(group = str_extract(Parameter,"(.+)\\[(\\d+)\\]", group = 1),
         taxa_id = str_extract(Parameter,"(.+)\\[(\\d+)\\]", group = 2))-> postSamples_df4
...
model{
  for (i in 1:D[1]) {    # For each OTU
  		for (j in 1:group[1]){ # I both group we test
  			Y[i,j] ~ dpois(exp(alpha[i] + log(N[j]) - pi[i,j]*c))
  			pi[i,j] ~ dbern(p[i]) # zero inflation
  		}
  	  	  
      for (j in (group[1]+1):(group[1]+group[2])){
  			Y[i,j] ~ dpois(exp(alpha[i] + beta[i] + log(N[j]) - pi[i,j]*c))
  			pi[i,j] ~ dbern(p[i]) # zero inflation
      }
...
data_list = list(
  ...
  group = c(50,50),
  N = colSums(Y)/median(colSums(Y)),
  c = 1000 # constant use to "activate" zero inflation in the model
)

init_list <- list(
  alpha = runif(n_taxa,2,4),
  beta = runif(n_taxa,-1,1),
  p = runif(n_taxa),
  pi = Y * 0
)
...
postSamples_df3%>%
  ...
  facet_wrap(~taxa_id)+
  geom_density(aes(value, fill = group), alpha = 0.3)+
  scale_fill_manual(values = wesanderson::wes_palette("FantasticFox1", n = 2))+
  main_theme

exp(quantile_summary3$statistics[1:10,1]) - exp(alpha) 
exp(quantile_summary3$statistics[11:20,1]) - exp(beta) 
...
Y <- meta_g_b_decond_table_rare%>%
   select(ends_with("R1_G"), ends_with("R1_B"))

Y <- Y[1:200,c(1:10, 80:89)]
...
model1 <- jags.model(textConnection(modelName),data = data_list,inits = init_list, n.chains = 1,n.adapt = nrAdapt)
str(post_sample)
post_sample <- coda.samples(model1,c("lambda"),n.iter = nrSamples,thin=nrThin)

postSamples_df <-  post_sample %>% ggs()
postSamples_df
postSamples_df%>%
  filter(Parameter %in% c("lambda[1]", "lambda[2]"))%>%
  ggplot() +
  facet_wrap(~Parameter)+
  geom_line(aes(Iteration, value , col = as.factor(Chain)), alpha = 0.3)+
  scale_color_manual(values = wesanderson::wes_palette("FantasticFox1", n = 5))
...
postSamples_df%>%
  ...
  ggplot()+
  geom_histogram(aes(mean_value))
```


### comparing mean abudance per OTU per group assuming poisson distibution
```{r}
modelName <- "model {
...
model1 <- jags.model(textConnection(modelName),data = data_list,inits = init_list, n.chains = 1,n.adapt = nrAdapt)
str(post_sample)
post_sample <- coda.samples(model1,c("lambda", "beta"),n.iter = nrSamples,thin=nrThin)

postSamples_df <-  post_sample %>% ggs()
postSamples_df%>%
  filter(Parameter %in% c("beta[1]", "beta[2]"))%>%
  ggplot() +
  facet_wrap(~Parameter)+
  geom_line(aes(Iteration, value , col = as.factor(Chain)), alpha = 0.3)+
  scale_color_manual(values = wesanderson::wes_palette("FantasticFox1", n = 5))
...
ZiMGLMMLoan <- function(Y,group,N,nR=dim(Y)[1],nrAdapt=1000,
  ...
  if(returnBeta1) {
    return(data.frame(betaProb = betaProb, betaMeans = betaMeans, betaSds = betaSds,pMeans = pMeans))
  } else {
    return(betaProb)
  }
...
Y <- meta_g_b_decond_table_rare%>%
   select(ends_with("R1_G"), ends_with("R1_B"))
#Note this is just an example that uses a small part
# of the Yatsunenko 2012 dataset to get a rapidly runnable test case.
Y <- Y[1:200,c(1:10, 80:89)] 
groups <- c(10,10) #Indicates the size of each group samples
...
normalizingConstants <- colSums(Y) #Uses total counts per sample
                                   #as normalization (with 0 as 1)
...
### Obs out of habit the median of the total sums are used.
normalizingConstants <- normalizingConstants/median(normalizingConstants)
...
model1 <- jags.model("code/ZiGLMM.jags",data = dataL,inits = initsL1, n.chains = 1,n.adapt = nrAdapt)
bf <- proc.time()
update(model1,n.iter=nrBurnIn)
...
#runs the mcmc for the selected settings and returns the significance score,
#fold change and zero inflation parameter for each feature
resultdf <- ZiMGLMMLoan(Y=Y,group = group, N = normalizingConstants,
                          nrAdapt=nrAdapt,nrBurnIn=nrBurnIn,nrSamples=nrSamples,
                          nrThin=nrThin,returnBeta=TRUE)
sigScore <- resultList[[1]]
foldChange <- resultList[[2]]
zeroInf <- resultList[[3]]
resOrder <- order(sigScore)


#export results
resultMat<-cbind(rownames(Y),sigScore,foldChange,zeroInf) 
resultMatOrdered<-cbind(rownames(Y)[resOrder],sigScore[resOrder],foldChange[resOrder],zeroInf[resOrder]) 
