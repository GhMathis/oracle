...
rarefaction.func = function(df,rar_sample, n_rare = 100, rarcur = T){
  ...
  if(rarcur){ # {{ if rarefaction curve needed}}
    metab1_decont_table %>%
      ...
      t() %>%
      rarecurve(step = 20, sample = rar_sample, col = "blue", cex = 0.6)
  }
   df %>%
    select(starts_with("BU")) %>%
    dim -> D
...
summarise_df%>%
  ...
  geom_smooth(aes(log10(reads_mean), log10(reads_var)), size= 1.5, method = "lm")+
...
For a $log_{10}(\mu) \approx 0 \rigtharrow log_{10}(\sigma^2) \approx 1.44$ for all method exept 2_G ($\log_{10}(mu_0) = 1.84$) (after species of mean and var under 1 are removed) (IDK what to interpret from that). The slope is arround 1 for R1_G and R2_G. For metagenomic species means are proportionnal to there variance ($\mu \prop \sigma^2$). For metabarcoding slopes are 1.34 for R1_B and R2_B, this tells us that for this method variances are propotional to a power law of means ($\mu^n \prop \sigma^2$ with $n = 1.35$ here)
...
obj1$data$tax_abund %>%
   filter(taxon_id == "afs") %>%
   select(ends_with("R1_B")) -  obj1$data$tax_abund %>%
                                   filter(taxon_id == "afs") %>%
                                   select(ends_with("R2_B")) 
obj1_rare$data$tax_abund %>%
   filter(taxon_id == "afs") %>%
   select(ends_with("R1_B")) - obj1_rare$data$tax_abund %>%
                                 filter(taxon_id == "afs") %>%
                                 select(ends_with("R2_B")) 
```
...
Zero inflation might bias the variance of taxa abundance in each method thus scaling factors are maybe under estimated
...
### bayesian poisson DGE zero inflated
...
names(obj3$data$diff_analysis_table_zif)[1] = "taxon_id"
names(obj3_rare$data$diff_analysis_table_zif)[1] = "taxon_id"

obj3_rare$data$diff_analysis_table_zif %>%
...
obj3_rare%>%
  ...
  metacoder::heat_tree(node_label = taxon_names,
          node_size = n_obs, # number of OTUs
          node_color = beta, # difference between groups
          node_color_interval = c(-max(abs(beta)), max(abs(beta))), # symmetric interval
          node_color_range = pal, # diverging colors
          node_size_axis_label = "OTU count",
          node_color_axis_label = "logFC (beta)",
          layout = "davidson-harel", initial_layout = "reingold-tilford",
...
# accounting for significance and convergence
obj3$data$diff_analysis_table_zif %>%
  select(taxon_id, beta_sign) %>%
  left_join(obj3_rare$data$diff_analysis_table_zif %>%
            select(taxon_id, beta_sign), by ="taxon_id" )%>% 
  ggplot()+
  geom_point(aes(beta_sign.x, beta_sign.y),size = 3, alpha =0.6)+
  geom_abline(intercept = 0, slope = 1, size = 2, col = "red")+
